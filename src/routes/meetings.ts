...
router.get("/meetings/:id", (req: AuthRequest, res) => {
  ...
  const tasks = db.prepare("SELECT id,title,status,priority,due_at FROM tasks WHERE meeting_id=? AND council_id=? ORDER BY created_at DESC").all(routeParam(req,"id"),council);
  const documents = db.prepare("SELECT id,title,category,created_at FROM documents WHERE meeting_id=? AND council_id=? ORDER BY created_at DESC").all(routeParam(req,"id"),council);
  res.json({ meeting, agenda, attendees, decisions, tasks, documents });
});
...
router.post("/meetings", permit("meetings:write"), (req: AuthRequest, res) => {
  ...
  assertOwned(db,c,"committees",d.committeeId,"Gremium");assertOwned(db,c,"users",d.chairId,"Sitzungsleitung");assertOwned(db,c,"users",d.secretaryId,"Schriftführung");
