...
router.get("/documents/:id",(req:AuthRequest,res)=>{const db=getDb(),id=routeParam(req,"id"),c=req.user!.councilId,item=db.prepare(`SELECT d.*,u.first_name,u.last_name FROM documents d LEFT JOIN users u ON u.id=d.created_by WHERE d.id=? AND d.council_id=?`).get(id,c) as Record<string,unknown>|undefined;if(!item)throw new HttpError(404,"Dokument nicht gefunden.");const versions=db.prepare(`SELECT v.id,v.version,v.original_name,v.mime_type,v.size,v.checksum,v.created_at,u.first_name,u.last_name FROM document_versions v LEFT JOIN users u ON u.id=v.uploaded_by WHERE v.document_id=? ORDER BY v.version DESC`).all(id),cases=item.case_id?db.prepare("SELECT id,reference,title,status,priority FROM cases WHERE id=? AND council_id=?").all(item.case_id,c):[],meetings=item.meeting_id?db.prepare("SELECT id,sequence_no,title,status,starts_at FROM meetings WHERE id=? AND council_id=?").all(item.meeting_id,c):[],agreements=item.agreement_id?db.prepare("SELECT id,reference,title,status FROM agreements WHERE id=? AND council_id=?").all(item.agreement_id,c):[];res.json({item,related:{cases,meetings,agreements,versions}});});

router.patch("/documents/:id",permit("documents:write"),(req:AuthRequest,res)=>{const d=z.object({title:z.string().trim().min(2).max(250).optional(),folder:z.string().trim().min(1).max(250).optional(),category:z.string().trim().min(1).max(100).optional(),confidentiality:z.enum(["gremium","ausschuss","streng_vertraulich"]).optional(),retentionUntil:z.string().max(40).optional().nullable(),caseId:optionalId,meetingId:optionalId,agreementId:optionalId}).parse(req.body),db=getDb(),id=routeParam(req,"id"),councilId=req.user!.councilId;if(!db.prepare("SELECT id FROM documents WHERE id=? AND council_id=?").get(id,councilId))throw new HttpError(404,"Dokument nicht gefunden.");assertOwned(db,councilId,"cases",d.caseId,"Vorgang");assertOwned(db,councilId,"meetings",d.meetingId,"Sitzung");assertOwned(db,councilId,"agreements",d.agreementId,"Vereinbarung");const map:Record<string,string>={title:"title",folder:"folder",category:"category",confidentiality:"confidentiality",retentionUntil:"retention_until",caseId:"case_id",meetingId:"meeting_id",agreementId:"agreement_id"},entries=Object.entries(d);if(entries.length)db.prepare(`UPDATE documents SET ${entries.map(([key])=>`${map[key]}=?`).join(",")},updated_at=CURRENT_TIMESTAMP WHERE id=? AND council_id=?`).run(...entries.map(([,value])=>value),id,councilId);audit(db,req,"updated","document",id,{fields:entries.map(([key])=>key)});res.status(204).end();});
...
router.post("/documents",permit("documents:write"),upload.single("file"),(req:AuthRequest,res)=>{
  ...
  const d=z.object({title:z.string().trim().min(2).max(250),folder:z.string().max(250).optional().default("Allgemein"),category:z.string().max(100).optional().default("sonstiges"),confidentiality:z.enum(["gremium","ausschuss","streng_vertraulich"]).optional().default("gremium"),caseId:z.string().uuid().optional().or(z.literal("")).transform(v=>v||null),meetingId:z.string().uuid().optional().or(z.literal("")).transform(v=>v||null),agreementId:z.string().uuid().optional().or(z.literal("")).transform(v=>v||null),retentionUntil:z.string().max(40).optional().or(z.literal("")).transform(v=>v||null)}).parse(req.body);
  const db=getDb(),councilId=req.user!.councilId;assertOwned(db,councilId,"cases",d.caseId,"Vorgang");assertOwned(db,councilId,"meetings",d.meetingId,"Sitzung");assertOwned(db,councilId,"agreements",d.agreementId,"Vereinbarung");const id=randomUUID(),versionId=randomUUID(),stored=storeEncrypted(file.buffer);
  db.transaction(()=>{
    db.prepare(`INSERT INTO documents (id,council_id,title,folder,category,confidentiality,case_id,meeting_id,agreement_id,retention_until,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)`).run(id,councilId,d.title,d.folder,d.category,d.confidentiality,d.caseId,d.meetingId,d.agreementId,d.retentionUntil,req.user!.id);
